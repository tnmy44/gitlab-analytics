"""
Propensity to Purchase Leads DAG
"""
...
# Create the DAG
# Run every the 9th day of the month at 6AM
dag = DAG(
    "ds_propensity_to_purchase_leads",
    default_args=default_args,
    schedule_interval="0 6 9 * *",
)

ptpf_scoring_command = f"""
    {clone_data_science_ptpf_repo_cmd} &&
    cd propensity-to-purchase/prod/lead-scoring &&
    papermill scoring_code.ipynb -p is_local_development False
"""
KubernetesPodOperator(
    **gitlab_defaults,
    image=ANALYST_IMAGE,
    task_id="propensity-to-purchase-leads",
    name="propensity-to-purchase-leads",
    secrets=[
        SNOWFLAKE_ACCOUNT,
        SNOWFLAKE_LOAD_PASSWORD,
        SNOWFLAKE_DATA_SCIENCE_LOAD_ROLE,

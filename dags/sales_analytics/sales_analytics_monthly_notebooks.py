...
# Create the DAG
# Schedule to run monthly at 6AM on every 7th day of the month
dag = DAG(
...
MONTHLY_NOTEBOOKS_PATH = f"{SALES_ANALYTICS_NOTEBOOKS_PATH}/monthly/"
...
for notebook, task_name in notebooks.items():
    ...
    container_cmd_load = f"""
        {clone_repo_cmd} &&
        cd {MONTHLY_NOTEBOOKS_PATH} &&
        papermill {notebook} -p is_local_development False
        """
    task_identifier = f"{task_name}"
    # Task 2
    sales_analytics_monthly_notebooks = KubernetesPodOperator(
        **gitlab_defaults,
        image=ANALYST_IMAGE,
    ...
    start >> sales_analytics_monthly_notebooks

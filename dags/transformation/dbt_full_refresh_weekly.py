...
dbt_full_refresh_cmd = f"""
    ...
    dbt run --profiles-dir profile --target prod --full-refresh --exclude common.dim_ip_to_geo | tee logs/logs.txt; ret=$?;
...
dbt_test_cmd = f"""
    {pull_commit_hash} &&
    {dbt_install_deps_cmd} &&
    dbt test --profiles-dir profile --target prod --exclude snowplow legacy.snapshots source:gitlab_dotcom source:salesforce source:zuora workspaces.* | tee logs/logs.txt; ret=$?;
    montecarlo import dbt-run --manifest target/manifest.json --run-results target/run_results.json --logs logs/logs.txt --project-name gitlab-analysis;
...
dbt_workspaces_test_command = f"""
    {pull_commit_hash} &&
    {dbt_install_deps_cmd} &&
    dbt test --profiles-dir profile --target prod --models workspaces.* | tee logs/logs.txt; ret=$?;
    montecarlo import dbt-run --manifest target/manifest.json --run-results target/run_results.json --logs logs/logs.txt --project-name gitlab-analysis;
    python ../../orchestration/upload_dbt_file_to_snowflake.py test; exit $ret
"""
...
dbt_results_cmd = f"""
    {pull_commit_hash} &&
    {dbt_install_deps_cmd} &&
    dbt run --profiles-dir profile --target prod --models sources.dbt+ | tee logs/logs.txt; ret=$?;
    montecarlo import dbt-run --manifest target/manifest.json --run-results target/run_results.json --logs logs/logs.txt --project-name gitlab-analysis;
    python ../../orchestration/upload_dbt_file_to_snowflake.py results; exit $ret
"""

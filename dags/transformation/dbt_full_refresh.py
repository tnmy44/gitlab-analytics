...
dbt_full_refresh_cmd = f"""
    ...
    dbt --no-use-colors run --profiles-dir profile --target prod --models {dbt_model_to_full_refresh} --full-refresh | dbt build | tee target/logs.txt; ret=$?;
    montecarlo import dbt-run --manifest target/manifest.json --run-results target/run_results.json --logs target/logs.txt --project-name gitlab-analysis;

...
def generate_dbt_command(vars_dict):
    ...
    dbt_generate_command = f"""
        ...
        dbt --debug --log-format json run --profiles-dir profile --target prod --models +snowplow --full-refresh --vars '{json_dict}'; ret=$?;
...
dbt_snowplow_combined_cmd = f"""
        {dbt_install_deps_nosha_cmd} &&
        dbt --debug --log-format json run --profiles-dir profile --target prod --models legacy.snowplow.combined; ret=$?;
        montecarlo import dbt-run --manifest target/manifest.json --run-results target/run_results.json --logs logs/dbt.log --project-name gitlab-analysis;
        python ../../orchestration/upload_dbt_file_to_snowflake.py results; exit $ret
        """

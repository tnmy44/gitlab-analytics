...
def generate_dbt_command(vars_dict):
    ...
    dbt_generate_command = f"""
        ...
        dbt run --profiles-dir profile --target prod --models +snowplow --full-refresh --vars '{json_dict}' | tee logs/logs.txt; ret=$?;
...
dbt_snowplow_combined_cmd = f"""
        {dbt_install_deps_nosha_cmd} &&
        dbt run --profiles-dir profile --target prod --models legacy.snowplow.combined | tee logs/logs.txt; ret=$?;
        montecarlo import dbt-run --manifest target/manifest.json --run-results target/run_results.json --logs logs/logs.txt --project-name gitlab-analysis;
        python ../../orchestration/upload_dbt_file_to_snowflake.py results; exit $ret
        """

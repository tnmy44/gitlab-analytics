...
dbt_non_product_models_command = f"""
    ...
    dbt --no-use-colors run --profiles-dir profile --target prod --exclude tag:product tag:six_hourly legacy.sheetload legacy.snapshots sources.gitlab_dotcom sources.sheetload sources.sfdc sources.zuora sources.dbt workspaces.* | tee logs/logs.txt; ret=$?;
...
dbt_product_models_command = f"""
    ...
    dbt --no-use-colors run --profiles-dir profile --target prod --models tag:product --exclude workspaces.* | tee logs/logs.txt; ret=$?;
    montecarlo import dbt-run --manifest target/manifest.json --run-results target/run_results.json --logs logs/logs.txt --project-name gitlab-analysis;
    python ../../orchestration/upload_dbt_file_to_snowflake.py results; exit $ret
"""
...
dbt_test_cmd = f"""
    ...
    dbt --no-use-colors test --profiles-dir profile --target prod --exclude snowplow legacy.snapshots source:gitlab_dotcom source:salesforce source:zuora workspaces.* | tee logs/logs.txt; ret=$?;
    montecarlo import dbt-run --manifest target/manifest.json --run-results target/run_results.json --logs logs/logs.txt --project-name gitlab-analysis;
...
dbt_results_cmd = f"""
    {pull_commit_hash} &&
    {dbt_install_deps_cmd} &&
    dbt --no-use-colors run --profiles-dir profile --target prod --models sources.dbt+ | tee logs/logs.txt; ret=$?;
    montecarlo import dbt-run --manifest target/manifest.json --run-results target/run_results.json --logs logs/logs.txt --project-name gitlab-analysis;
    python ../../orchestration/upload_dbt_file_to_snowflake.py results; exit $ret
"""
...
dbt_workspaces_command = f"""
    ...
    {dbt_install_deps_cmd} &&
    export SNOWFLAKE_TRANSFORM_WAREHOUSE="TRANSFORMING_XL" &&
    dbt --no-use-colors run --profiles-dir profile --target prod --models workspaces.* --exclude workspaces.workspace_data_science.* workspaces.workspace_data.tdf.* | tee logs/logs.txt; ret=$?;
    montecarlo import dbt-run --manifest target/manifest.json --run-results target/run_results.json --logs logs/logs.txt --project-name gitlab-analysis;
    python ../../orchestration/upload_dbt_file_to_snowflake.py results; exit $ret
"""
...
dbt_workspaces_xl_command = f"""
    ...
    {dbt_install_deps_cmd} &&
    export SNOWFLAKE_TRANSFORM_WAREHOUSE="TRANSFORMING_L" &&
    dbt --no-use-colors run --profiles-dir profile --target prod --models workspaces.workspace_data_science.* | tee logs/logs.txt; ret=$?;
    montecarlo import dbt-run --manifest target/manifest.json --run-results target/run_results.json --logs logs/logs.txt --project-name gitlab-analysis;
    python ../../orchestration/upload_dbt_file_to_snowflake.py results; exit $ret
"""
...
dbt_workspaces_test_command = f"""
    {pull_commit_hash} &&
    {dbt_install_deps_cmd} &&
    dbt --no-use-colors test --profiles-dir profile --target prod --models workspaces.* | tee logs/logs.txt; ret=$?;
    montecarlo import dbt-run --manifest target/manifest.json --run-results target/run_results.json --logs logs/logs.txt --project-name gitlab-analysis;
    python ../../orchestration/upload_dbt_file_to_snowflake.py test; exit $ret
"""

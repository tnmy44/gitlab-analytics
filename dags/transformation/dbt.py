...
dbt_non_product_models_command = f"""
    ...
    dbt --no-use-colors run --profiles-dir profile --target prod --exclude tag:product tag:six_hourly tag:gitlab_dotcom_backfill_chunked legacy.sheetload legacy.snapshots sources.gitlab_dotcom sources.sheetload sources.sfdc sources.zuora sources.dbt workspaces.* ; ret=$?;

...
class TableauAllDependencyCheck:
    """
    Class for checking tableau dependencies
    """
    ...
    def check_tableau_dependencies(self, model_ids: List):
        ...
        for line in model_ids:
            ...
            if full_table_path is None:
                ...
                source_table_mcon = query_table(full_table_path)
                response_downstream_node_dependencies = (
                    get_all_downstream_node_dependencies(source_table_mcon)
                )
                ...
                if response_downstream_node_dependencies is None:
                    ...
                    output_list = check_response_for_tableau_all_dependencies(
                        response_downstream_node_dependencies
                    )
...
class DirectQueryCheck:
    """
    Class for direct queries against the table that are from team members
    """

    def check_query_dependencies(self, model_ids: List):
        """
        :param model_input:
        :return:
        """
        has_dependency = False
        for line in model_ids:
            logging.info(f"Checking for queries against for the model {line.strip()}")
            full_table_path = get_table_path_query(line)
            # if no path is returned exit the script
            if full_table_path is None:
                logging.info(f"No dependencies returned for model {format(line)}")
                ...
                source_table_mcon = query_table(full_table_path)
                response_downstream_node_dependencies = (
                    get_all_queries_node_dependencies(source_table_mcon)
                )
                ...
                if response_downstream_node_dependencies is None:
                    ...
                    output_list = check_response_for_query_dependencies(
                        response_downstream_node_dependencies
                    )
...
def get_all_queries_node_dependencies(table_mcon: str) -> dict:
    """
    This will return all dependent downstream tableau nodes for a given model.
    """
    report_types = [
        "tableau-workbook",
    ...
    query_type = "read"  # "source"
    start_time = datetime.datetime.now() - datetime.timedelta(days=30)
    end_time = datetime.datetime.now()

    json = {
        # "query": "query GetQueryList($mcon: String!, $queryType: String!, $startTime: DateTime!, $endTime: DateTime!) {getQueryList(mcon: $mcon,queryType: $queryType,startTime: $startTime,endTime: $endTime) {queries {subCategory,timestamp,userName}}}",
        "query": "query GetQueryList($mcon: String!, $queryType: QueryType!, $startTime: DateTime!, $endTime: DateTime!) {getQueryDimensions(mcon: $mcon,queryType: $queryType,startTime: $startTime,endTime: $endTime) {categories users }}",
        "variables": {
            "queryType": f"{query_type}",
            "startTime": f"{start_time.strftime('%Y-%m-%dT%H:%M:%S')}",
            "endTime": f"{end_time.strftime('%Y-%m-%dT%H:%M:%S')}",
            "mcon": f"{table_mcon}",
        },
    }
    response_content = get_response(json)
    # response_derived_tables_partial_lineage = response_content["data"]["getQueryList"][0]
    response_derived_tables_partial_lineage = response_content["data"][
        "getQueryDimensions"
    ]
...
def check_response_for_query_dependencies(
    ...
    """
    dependency_list = []
    for user in response_downstream_dependencies["users"]:
        output_dict = {}
        exception_users = ["AIRFLOW", "GITLAB_CI", "TABLEAU"]
        if user not in exception_users:

            object_type_resource_name = f"{user} - "

            output_dict[object_type_resource_name] = f"filler"

            dependency_list.append(output_dict)
    """dependency_list = []
    for node in response_downstream_dependencies["queries"]:
        output_dict = {}
        exception_users = [
            "AIRFLOW",
            "GITLAB_CI",
        ]
        if node["userName"] not in exception_users:

            object_type_resource_name = f"{node['userName']} : {node['subCategory']} - "

            output_dict[object_type_resource_name] = f"{node['timestamp']}"
...
def get_all_downstream_node_dependencies(table_mcon: str) -> dict:
    """
    This will return all dependent downstream tableau nodes for a given model.
    """
    report_types = [
    ...
    json = {
        "query": "query GetDownstreamReports($mcon:[String]!,$report_types:[String]) {getDownstreamReports(mcons:$mcon, reportTypes:$report_types){downstreamReports{displayName,mcon,reportType}}}",
        "variables": {"report_types": report_types, "mcon": f"{table_mcon}"},
    }
    response_content = get_response(json)
    response_derived_tables_partial_lineage = response_content["data"][
...
def check_response_for_tableau_all_dependencies(
    response_downstream_dependencies: dict,
) -> list:
    """
    This will return all dependent downstream nodes for a given source table.
    """
    dependency_list = []
    for node in response_downstream_dependencies["downstreamReports"]:
        output_dict = {}
        object_type_resource_name = f"{node['reportType']} : {node['displayName']} - "

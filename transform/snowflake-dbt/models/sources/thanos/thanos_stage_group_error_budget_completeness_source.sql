...
parsed AS (
  ...
  FROM
    source,
    LATERAL FLATTEN(input => source.jsontext['stage_group_error_budget_teams_over_budget_availability']) parse_json_1,
    LATERAL FLATTEN(input => source.jsontext['stage_group_error_budget_teams_over_budget_availability']['body']['data']['result'], outer => TRUE) parse_json_2
  WHERE result_type IS NOT NULL AND status_type IS NOT NULL
)

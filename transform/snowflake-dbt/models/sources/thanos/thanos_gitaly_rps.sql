WITH source AS (
  ...
  FROM
    source INNER JOIN
    LATERAL FLATTEN(input => source.jsontext['gitaly_rate_5m']) jsonpath_root INNER JOIN
    LATERAL FLATTEN(input => source.jsontext['gitaly_rate_5m']['body']['data']['result'], outer => TRUE) jsonpath_result

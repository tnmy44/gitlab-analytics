...
fct_event AS (
    ...
    /*
    This CTE pulls in ALL of the columns from the fct_event as a base data set. It uses the dbt_utils.star 
    function to select all columns except the metadata table related columns from fct_event. To improve 
    performance and reduce the number of records in the subsequent JOINs, some of the fct_event_valid 
    filters are captured here.
    This CTE filters out imported projects and events with data quality issues by filtering out negative 
    days since user creation at event date. It keeps events with a NULL days since user creation to capture 
    events that do not have a user. This CTE also filters to a rolling 36 months of data for performance 
    optimization.
    */
...
fct_event_valid AS (
    ...
    */
    ...
    WHERE (dim_user.is_blocked_user = FALSE 

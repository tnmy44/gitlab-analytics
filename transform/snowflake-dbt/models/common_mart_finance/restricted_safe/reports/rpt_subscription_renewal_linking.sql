...
-- marking the last version with row number
  ...
  LEFT JOIN prod.common.dim_billing_account ON dim_subscription.dim_billing_account_id = dim_billing_account.dim_billing_account_id
...
-- opportunity basis data for renewals where opportunity is renewal and quote is new business, e.g. late renewal
  ...
  LEFT JOIN prod.restricted_safe_common.fct_quote ON mart_crm_opportunity.dim_crm_opportunity_id = fct_quote.dim_crm_opportunity_id
  LEFT JOIN prod.common.dim_quote ON fct_quote.dim_quote_id = dim_quote.dim_quote_id
...
-- final model for opportunities renewal and quotes new subscriptions with suggested prior subscriptions, use case e.g. late renewal, debook/rebook, contract reset
  ...
  LEFT JOIN last_version_non_cancelled ON opportunity_basis_renewal.dim_crm_account_id = last_version_non_cancelled.dim_crm_account_id
  WHERE (
...
-- final model for opportunities new business and quotes new subscriptions with suggested prior subscriptions, use case e.g. legacy ramp
  ...
  LEFT JOIN last_version_non_cancelled ON opportunity_basis_new_business.dim_crm_account_id = last_version_non_cancelled.dim_crm_account_id
  WHERE (
...
entity AS (
  ...
  LEFT JOIN first_version_non_cancelled ON opportunity_basis_entity.dim_crm_account_id = first_version_non_cancelled.dim_crm_account_id
  LEFT JOIN last_version_non_cancelled ON opportunity_basis_entity.subscription_name = last_version_non_cancelled.subscription_name

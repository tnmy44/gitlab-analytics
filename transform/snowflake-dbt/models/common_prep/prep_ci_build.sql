...
, gitlab_dotcom_ci_builds_source AS (
    ...
    SELECT
      ...
      WHEN failure_reason = 4 THEN 'runner_or_system_failure'
      WHEN failure_reason = 5 THEN 'missing_dependency_failure'
      WHEN failure_reason = 6 THEN 'runner_unsupported'
      WHEN failure_reason = 7 THEN 'stale_schedule'
      WHEN failure_reason = 8 THEN 'job_execution_timeout'
      WHEN failure_reason = 9 THEN 'archived_failure' 
      WHEN failure_reason = 10 THEN 'unmet_prerequisites'
      WHEN failure_reason = 11 THEN 'scheduler_failure'
      WHEN failure_reason = 12 THEN 'data_integrity_failure'
      WHEN failure_reason = 13 THEN 'forward_deployment_failure'
      WHEN failure_reason = 14 THEN 'user_blocked'
      WHEN failure_reason = 15 THEN 'project_deleted'
      WHEN failure_reason = 16 THEN 'ci_quota_exceeded'
      WHEN failure_reason = 17 THEN 'pipeline_loop_detected'
      WHEN failure_reason = 18 THEN 'no_matching_runner'
      WHEN failure_reason = 19 THEN 'trace_size_exceeded'
      WHEN failure_reason = 20 THEN 'builds_disabled'
      WHEN failure_reason = 21 THEN 'environment_creation_failure'
      WHEN failure_reason = 22 THEN 'deployment_rejected'
      WHEN failure_reason = 23 THEN 'failed_outdated_deployment_job'
      WHEN failure_reason = 10000 THEN 'protected_environment_failure'
      WHEN failure_reason = 10001 THEN 'insufficient_bridge_permissions'
      WHEN failure_reason = 10002 THEN 'downstream_bridge_project_not_found'
      WHEN failure_reason = 10003 THEN 'invalid_bridge_trigger'
      WHEN failure_reason = 10004 THEN 'upstream_bridge_project_not_found'
      WHEN failure_reason = 10005 THEN 'insufficient_upstream_permissions'
      WHEN failure_reason = 10006 THEN 'bridge_pipeline_is_child_pipeline'
      WHEN failure_reason = 10007 THEN 'donstream_pipeline_creation_failed'
      WHEN failure_reason = 10008 THEN 'secrets_provider_not_found'
      WHEN failure_reason = 10009 THEN 'reached_max_descendant_pipelines_depth'
      WHEN failure_reason = 10010 THEN 'ip_restriction_failure'
      WHEN failure_reason = 10011 THEN 'reached_max_pipeline_hierarchy_size'
      WHEN failure_reason = 10012 THEN 'reached_downstream_pipeline_trigger_rate_limit'
      ELSE null 
      END as failure_reasion,

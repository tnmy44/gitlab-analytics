...
, fiscal_months AS (
    ...
    SELECT
      ...
      CASE
        ...
        WHEN fiscal_months.fiscal_year >= 2024 
          AND (sheetload_sales_funnel_targets_matrix_source.user_business_unit IS NOT NULL AND LOWER(sheetload_sales_funnel_targets_matrix_source.user_business_unit) NOT IN ('comm', 'entg')) -- account for non-sales reps
          THEN CONCAT(
                      UPPER(sheetload_sales_funnel_targets_matrix_source.user_business_unit), 
                      '-',
                      UPPER(sheetload_sales_funnel_targets_matrix_source.user_segment), 
    ...
    INNER JOIN fiscal_months
      ON {{ sales_funnel_text_slugify("sheetload_sales_funnel_targets_matrix_source.month") }} = {{ sales_funnel_text_slugify("fiscal_months.fiscal_month_name_fy") }}

...
LEFT JOIN cw_base
  ON abm_tier_id_final.dim_crm_opportunity_id=cw_base.dim_crm_opportunity_id    
    ...
    SELECT DISTINCT
      ...
      -- Pull live values for geo fields: https://gitlab.com/gitlab-data/analytics/-/issues/18976
    ...
    END                                                         AS cycle_time_in_days,
      ...
      -- This code calculates quarter-specific sales metrics
      -- and tracks their progression within the snapshot fiscal quarter.
      
      -- Generated Pipeline in a specific snapshot quarter
      CASE
      ...
      -- Number of booked deals in a specific snapshot quarter  
      CASE
      ...
      --  Number of closed opportunities (lost and won) in a specific snapshot quarter 
      CASE
        WHEN sfdc_opportunity.snapshot_fiscal_quarter_date = close_fiscal_quarter_date
          AND is_win_rate_calc = TRUE
            THEN calculated_deal_count
      ...
      -- Booked net ARR in a specific snapshot quarter
      CASE
        WHEN sfdc_opportunity.snapshot_fiscal_quarter_date = close_fiscal_quarter_date
      ...
      -- Number of deals booked in a specific snapshot quarter 
      CASE
        WHEN sfdc_opportunity.snapshot_fiscal_quarter_date = close_fiscal_quarter_date
          AND is_booked_net_arr = TRUE 
          THEN calculated_deal_count
      ...
      -- Number of deals generated in a specific snapshot quarter 
      CASE
        WHEN sfdc_opportunity.snapshot_fiscal_quarter_date = arr_created_fiscal_quarter_date
          AND is_net_arr_pipeline_created = 1 
            THEN calculated_deal_count 
      ...
      -- Cycle time of opportunities closed in a specific snapshot quarter
      CASE
        WHEN sfdc_opportunity.snapshot_fiscal_quarter_date = close_fiscal_quarter_date 
          AND is_renewal = 1 
            AND  is_eligible_age_analysis = 1
      ...
      -- Net ARR of open stage 1+ opportunities that are planned to close within a specific snapshot quarter
      CASE
        WHEN sfdc_opportunity.snapshot_fiscal_quarter_date = close_fiscal_quarter_date 
      ...
      -- Net ARR of open stage 3+ opportunities that are planned to close within a specific snapshot quarter
      CASE
        WHEN sfdc_opportunity.snapshot_fiscal_quarter_date = close_fiscal_quarter_date 
          AND is_eligible_open_pipeline = 1
            AND is_stage_3_plus = 1
      ...
      -- Net ARR of open stage 4+ opportunities that are planned to close within a specific snapshot quarter
      CASE
        WHEN sfdc_opportunity.snapshot_fiscal_quarter_date = close_fiscal_quarter_date 
          AND is_eligible_open_pipeline = 1
            AND is_stage_4_plus = 1
      ...
      -- Number of open stage 1+ opportunities that are planned to close within a specific snapshot quarter 
      CASE 
        WHEN sfdc_opportunity.snapshot_fiscal_quarter_date = close_fiscal_quarter_date  
          AND is_eligible_open_pipeline = 1
            THEN calculated_deal_count
      ...
      -- They only include positive ARR opportunities. 
      CASE
        WHEN sfdc_opportunity.snapshot_fiscal_quarter_date = close_fiscal_quarter_date
          AND is_booked_net_arr = TRUE 
	          AND net_arr > 0

...
snapshot_dates AS (
  ...
  SELECT *
  FROM dim_date
  WHERE CAST (date_actual AS DATE) >= '2020-02-01' -- Restricting snapshot model to only have data from this date forward. More information https://gitlab.com/gitlab-data/analytics/-/issues/14418#note_1134521216
...
sfdc_opportunity_snapshot AS (
  ...
  SELECT
    ...
    CAST (sfdc_opportunity_snapshots_source.created_date AS DATE)                                                                                           AS created_date,
    CAST (sfdc_opportunity_snapshots_source.sales_accepted_date AS DATE)                                                                                    AS sales_accepted_date,
    CAST (sfdc_opportunity_snapshots_source.close_date AS DATE)                                                                                             AS close_date,
    ...
    CASE
      WHEN sfdc_opportunity_snapshots_source.stage_name IN (
  ...
  INNER JOIN snapshot_dates
    ON CAST (sfdc_opportunity_snapshots_source.dbt_valid_from AS DATE) <= snapshot_dates.date_actual
      AND (CAST (sfdc_opportunity_snapshots_source.dbt_valid_to AS DATE) > snapshot_dates.date_actual OR sfdc_opportunity_snapshots_source.dbt_valid_to IS NULL)
...
sfdc_opportunity_live AS (
  ...
  SELECT
    ...
    CAST (sfdc_opportunity_source.created_date AS DATE)                                                                               AS created_date,
    CAST (sfdc_opportunity_source.sales_accepted_date AS DATE)                                                                        AS sales_accepted_date,
    CAST (sfdc_opportunity_source.close_date AS DATE)                                                                                 AS close_date,
    ...
    sfdc_account.dim_parent_crm_account_id,
    CASE
      WHEN sfdc_opportunity_source.stage_name IN (
          '8-Closed Lost', 'Closed Lost', '9-Unqualified',
          'Closed Won', '10-Duplicate'
        )
        THEN 0
...
sao_base AS (
  ...
  SELECT
    ...
    COALESCE (sfdc_opportunity.is_edu_oss = 0
    AND sfdc_opportunity.stage_name != '10-Duplicate'
    AND sales_accepted_date BETWEEN valid_from AND valid_to, FALSE) AS is_abm_tier_sao
...
cw_base AS (
  ...
  SELECT
    ...
    COALESCE (stage_name = 'Closed Won'
    AND close_date BETWEEN valid_from AND valid_to, FALSE) AS is_abm_tier_closed_won
  FROM sfdc_opportunity
  LEFT JOIN account_history_final
    ON sfdc_opportunity.dim_crm_account_id = account_history_final.dim_crm_account_id
  WHERE abm_tier IS NOT NULL
...
final AS (
  ...
  SELECT DISTINCT
    ...
    CASE
      WHEN CAST (sfdc_opportunity.snapshot_date AS DATE) < '2021-02-01' -- All deals before cutoff and that were not updated to Net ARR
        THEN calculated_from_ratio_net_arr
      WHEN CAST (sfdc_opportunity.snapshot_date AS DATE) >= '2021-02-01'  -- After cutoff date, for all deals earlier than FY19 that are closed and have no net arr
        AND CAST (sfdc_opportunity.close_date AS DATE) < '2018-02-01'
    ...
    COALESCE (sfdc_opportunity.opportunity_term_base, DATEDIFF('month', quote.quote_start_date, sfdc_opportunity.subscription_end_date))                                                               AS opportunity_term,
    ...
    -- flags
    COALESCE (sfdc_opportunity.sales_accepted_date IS NOT NULL
    AND sfdc_opportunity_live.is_edu_oss = 0
    AND sfdc_opportunity.stage_name != '10-Duplicate', FALSE)                                                                         AS is_sao,
    COALESCE (is_sao = TRUE
    ...
    AND (sfdc_opportunity_live.is_jihu_account != TRUE OR sfdc_opportunity_live.is_jihu_account IS NULL), FALSE)                      AS is_net_arr_closed_deal,
    COALESCE ((
      sfdc_opportunity.new_logo_count = 1
      OR sfdc_opportunity.new_logo_count = -1
    )
    AND (sfdc_opportunity_live.is_jihu_account != TRUE OR sfdc_opportunity_live.is_jihu_account IS NULL), FALSE)                      AS is_new_logo_first_order,
    ...
    COALESCE (sfdc_opportunity.close_date <= CURRENT_DATE()
    ...
    AND sfdc_opportunity.stage_name NOT IN ('10-Duplicate', '9-Unqualified'), FALSE)                                                  AS is_win_rate_calc,
    COALESCE (sfdc_opportunity_stage.is_won = 'TRUE'
    AND sfdc_opportunity.is_closed = 'TRUE'
    AND sfdc_opportunity_live.is_edu_oss = 0, FALSE)                                                                                  AS is_closed_won,
    CASE
    ...
    CASE
      WHEN sfdc_opportunity.close_date <= CURRENT_DATE()
        ...
        AND (sfdc_opportunity_live.opportunity_category IN ('Standard') OR (
          ...
          sfdc_opportunity_live.opportunity_category = 'Ramp Deal'
          AND LEFT(sfdc_opportunity.dim_crm_opportunity_id, LENGTH(sfdc_opportunity.ssp_id)) = sfdc_opportunity.ssp_id
        ))
        THEN 1
      ELSE 0
    ...
    {{ alliance_partner_current('sfdc_opportunity.fulfillment_partner_account_name', 'sfdc_opportunity.partner_account_account_name', 'sfdc_opportunity.partner_track', 'sfdc_opportunity.resale_partner_track', 'sfdc_opportunity.deal_path', 'sfdc_opportunity.is_focus_partner') }}                                                                                                                                   
                                                                                                                                      AS alliance_type_current,
    {{ alliance_partner_short_current('sfdc_opportunity.fulfillment_partner_account_name', 'sfdc_opportunity.partner_account_account_name', 'sfdc_opportunity.partner_track', 'sfdc_opportunity.resale_partner_track', 'sfdc_opportunity.deal_path', 'sfdc_opportunity.is_focus_partner') }}                                                                                                                                   
                                                                                                                                      AS alliance_type_short_current,

    {{ alliance_partner('sfdc_opportunity.fulfillment_partner_account_name', 'sfdc_opportunity.partner_account_account_name', 'sfdc_opportunity.close_date', 'sfdc_opportunity.partner_track', 'sfdc_opportunity.resale_partner_track', 'sfdc_opportunity.deal_path', 'sfdc_opportunity.is_focus_partner') }}                                                                                                                                   
                                                                                                                                      AS alliance_type,
    {{ alliance_partner_short('sfdc_opportunity.fulfillment_partner_account_name', 'sfdc_opportunity.partner_account_account_name', 'sfdc_opportunity.close_date', 'sfdc_opportunity.partner_track', 'sfdc_opportunity.resale_partner_track', 'sfdc_opportunity.deal_path', 'sfdc_opportunity.is_focus_partner') }}                                                                                                                                   
    ...
    CASE
      ...
      --remove test accounts
      WHEN (
        sfdc_opportunity.dim_parent_crm_account_id = ('0016100001YUkWVAA1')
    ...
    UPPER(
      IFF(
        sfdc_opportunity_live.close_date < close_date_live.current_first_day_of_fiscal_year, sfdc_opportunity_live.dim_crm_user_hierarchy_account_user_sk,
        CONCAT(
  ...
  LEFT JOIN sfdc_opportunity_live
    ON sfdc_opportunity.dim_crm_opportunity_id = sfdc_opportunity_live.dim_crm_opportunity_id
  ...
  LEFT JOIN dim_date AS arr_created_date
    ON CAST (sfdc_opportunity.iacv_created_date AS DATE) = arr_created_date.date_actual
  LEFT JOIN dim_date AS subscription_start_date
    ON CAST (sfdc_opportunity.subscription_start_date AS DATE) = subscription_start_date.date_actual

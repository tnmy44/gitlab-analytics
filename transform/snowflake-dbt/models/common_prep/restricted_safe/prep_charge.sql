...
non_manual_charges AS (
  ...
  SELECT
    ...
    zuora_rate_plan_charge.created_date                                                                                                   AS charge_created_datetime,
    ...
    LAG(zuora_rate_plan_charge.mrr, 1)
      OVER (
        PARTITION BY zuora_subscription.subscription_name, zuora_rate_plan_charge.rate_plan_charge_number
        ORDER BY zuora_rate_plan_charge.segment, zuora_subscription.version
      )
      AS previous_mrr_calc,
    COALESCE (previous_mrr_calc, 0)                                                                                                       AS previous_mrr,
    ...
    LAG(zuora_rate_plan_charge.quantity, 1)
      OVER (
        PARTITION BY zuora_subscription.subscription_name, zuora_rate_plan_charge.rate_plan_charge_number
        ORDER BY zuora_rate_plan_charge.segment, zuora_subscription.version
      )
...
manual_charges AS (
  ...
  SELECT
    ...
    COALESCE (DATE_TRUNC('month', effective_end_date) > DATE_TRUNC('month', effective_start_date) OR DATE_TRUNC('month', effective_end_date) IS NULL, FALSE) AS is_included_in_arr_calc,

...
, latest_score_date AS (
    ...
    WITH combined_score_dates AS (
        SELECT dim_marketing_contact_id, score_date
...
select COALESCE(prep_ptpt_scores_by_user.dim_marketing_contact_id, prep_ptpf_scores_by_user.dim_marketing_contact_id, prep_ptpt_scores_by_user.dim_marketing_contact_id) as dim_marketing_contact_id_combined
     , COALESCE(prep_ptpt_scores_by_user.score_date, prep_ptpf_scores_by_user.score_date, prep_ptpl_scores_by_user.score_date) as score_date_combined

    /*
    -- Logic is as follows:
      - Trials: Because trials are updated daily, we can use the daily score when determining which model (ptpt, ptpf, ptpl) to use. Once the trial is over, their score will revert to NULL for their trial score
      - Free: Updated monthly. Therefore we will want to pull use the most recent Free score on or before the selected date when determining which model (ptpt, ptpf, ptpl) to use. This is why we are using the LAG function with IGNORE NULLS
      - Leads: Same logic as free
      - Once the historic ptpt, ptpf, and ptpl scores for each date are determined, then the logic is applied to determine which score actually gets populated in Salesforce
    */
    
    , prep_ptpt_scores_by_user.score_group as latest_ptpt 
    , COALESCE(prep_ptpf_scores_by_user.score_group, LAG(prep_ptpf_scores_by_user.score_group, 1,NULL) IGNORE NULLS OVER (PARTITION BY dim_marketing_contact_id_combined ORDER BY score_date_combined)) as latest_ptpf
...
ORDER BY dim_marketing_contact_id_combined, score_date_combined
    ...
    select dim_marketing_contact_id_combined    as dim_marketing_contact_id
         ...
         , CASE WHEN score_date = latest_score_date THEN NULL ELSE DATEADD('day', -1, valid_to_prep) END AS valid_to

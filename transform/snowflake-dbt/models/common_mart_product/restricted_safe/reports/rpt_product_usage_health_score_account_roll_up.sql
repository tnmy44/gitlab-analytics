...
SELECT DISTINCT
	...
	,SUM(CASE WHEN product_category = 'Base Products' THEN arr END) AS total_account_base_products_arr
	,SUM(CASE WHEN product_tier_name ilike '%Ultimate%' THEN arr END) AS total_account_ultimate_arr
	,SUM(CASE WHEN product_category <> 'Base Products' THEN arr END) AS total_account_add_on_arr
...
SELECT DISTINCT
	...
	,(CASE WHEN account_age_months < 3 THEN NULL
		  WHEN account_age_months >= 3 AND account_age_months < 7 THEN 	(CASE WHEN account_weighted_license_utilization <= 0.10 THEN 25
		      																  ...
		      																  ELSE NULL END)
		  WHEN account_age_months >= 7 AND account_age_months < 10 THEN (CASE WHEN account_weighted_license_utilization <= 0.50 THEN 25
    ...
    ,CASE WHEN account_level_license_utilization_score IS NULL THEN NULL
          ...
          WHEN account_level_license_utilization_score = 88 THEN 'Green' END AS account_level_license_utilization_color
    ,CASE WHEN account_weighted_user_engagement IS NULL THEN NULL
          ...
          WHEN account_weighted_user_engagement < 0.50 THEN 'Red' END AS account_level_user_engagement_color
	,CASE WHEN account_weighted_scm_utilization >= 0.50 THEN 'Green'
		  ...
		  ELSE 'No Data' END AS account_level_scm_color
	,CASE WHEN account_weighted_ci_utilization > 0.333 THEN 'Green'
		  ...
		  ELSE 'No Data' END AS account_level_ci_color
	,CASE WHEN account_weighted_cd_utilization > 75 THEN 'Green'
		  ...
		  ELSE 'No Data' END AS account_level_cd_color
	,CASE WHEN total_account_ultimate_arr IS NULL THEN 'N/A'
		  ...
		  ELSE 'No Data' END AS account_level_security_color
...
FULL OUTER JOIN account_rollup_calculations
	ON account_rollup_calculations.dim_crm_account_id = rpt_product_usage_health_score.dim_crm_account_id
		AND account_rollup_calculations.arr_month = rpt_product_usage_health_score.snapshot_month
)

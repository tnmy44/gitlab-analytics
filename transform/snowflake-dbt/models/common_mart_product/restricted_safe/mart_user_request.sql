...
, opportunity_seats AS (
    ...
    FROM epic_labels
    WHERE group_label IS NOT NULL
    QUALIFY ROW_NUMBER() OVER(PARTITION BY epic_id ORDER BY group_label) = 1
    ...
    FROM epic_labels
    WHERE product_group_extended IS NOT NULL
    QUALIFY ROW_NUMBER() OVER(PARTITION BY epic_id ORDER BY product_group_level DESC) = 1
    ...
    FROM epic_labels
    WHERE category_label IS NOT NULL
    QUALIFY ROW_NUMBER() OVER(PARTITION BY epic_id ORDER BY category_label DESC) = 1
    ...
    FROM epic_labels
    WHERE type_label IS NOT NULL
    QUALIFY ROW_NUMBER() OVER(PARTITION BY epic_id ORDER BY type_label) = 1
    ...
    FROM epic_labels
    WHERE devops_label IS NOT NULL
    QUALIFY ROW_NUMBER() OVER(PARTITION BY epic_id ORDER BY devops_label) = 1
    ...
    SELECT
      ...
      milestone_due_date
    FROM dim_issue
    QUALIFY ROW_NUMBER() OVER(PARTITION BY epic_id ORDER BY milestone_due_date DESC NULLS LAST) = 1

), user_request AS (

    SELECT
      bdg_issue_user_request.dim_issue_id                                         AS dim_issue_id,
      IFNULL(dim_issue.epic_id, -1)                                           AS epic_id,
    ...
    LEFT JOIN dim_epic
      ON dim_epic.epic_id = dim_issue.epic_id
    ...
    SELECT
      -1                                                                          AS dim_issue_id,
      bdg_epic_user_request.epic_id                                               AS epic_id,
    ...
    LEFT JOIN dim_issue
      ON dim_issue.epic_id = bdg_epic_user_request.epic_id
    LEFT JOIN dim_epic
      ON dim_epic.epic_id = bdg_epic_user_request.epic_id
    LEFT JOIN epic_last_milestone
      ON epic_last_milestone.epic_id = bdg_epic_user_request.epic_id
    LEFT JOIN epic_group_extended_label
      ON epic_group_extended_label.epic_id = bdg_epic_user_request.epic_id
    LEFT JOIN epic_weight
      ON epic_weight.epic_id = bdg_epic_user_request.epic_id
    ...
    LEFT JOIN epic_category_dedup AS category_label
      ON category_label.epic_id = bdg_epic_user_request.epic_id
    LEFT JOIN epic_group_label AS group_label
      ON group_label.epic_id = bdg_epic_user_request.epic_id
    LEFT JOIN epic_devops_label AS devops_label
      ON devops_label.epic_id = bdg_epic_user_request.epic_id
    LEFT JOIN epic_type_label AS type_label
      ON type_label.epic_id = bdg_epic_user_request.epic_id
    LEFT JOIN epic_theme_labels
      ON epic_theme_labels.epic_id = bdg_epic_user_request.epic_id

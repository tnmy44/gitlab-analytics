...
CASE WHEN e.dim_namespace_id IS NULL THEN 'Self-Managed' ELSE 'Gitlab.com' END AS likely_source, 
...
WHERE  
behavior_at BETWEEN '2024-03-21' AND CURRENT_DATE --first date of these events
AND
e.event_action = 'request_duo_chat_response'
...
unpacked u
JOIN
{{ ref('dim_namespace') }} n ON u.namespace_id = n.dim_namespace_id
WHERE
...
cs_prep 
JOIN 
{{ ref('dim_namespace') }} n ON n.dim_namespace_id = cs_prep.namespace_id
GROUP BY ALL 
), mapper_ai AS 
(
SELECT
...
{{ ref('dim_namespace') }} n
LEFT JOIN 
cs ON cs.ultimate_parent_namespace_id = n.ultimate_parent_namespace_id
LEFT JOIN 
duo_enabled d ON d.ultimate_parent_namespace_id = n.ultimate_parent_namespace_id
JOIN 
{{ ref('gitlab_dotcom_members') }} m ON m.source_id = n.ultimate_parent_namespace_id AND m.access_level = 50 AND m.is_currently_valid = TRUE
WHERE
n.namespace_is_ultimate_parent = TRUE 
AND
n.namespace_is_internal != TRUE
AND
(cs.ultimate_parent_namespace_id IS NOT NULL OR d.ultimate_parent_namespace_id IS NOT NULL)
), duo_purchasers AS 
(
SELECT 
...
MAX( m.owner_user_id) AS owner_user_id, -- CHANGE THIS by removing the comments to deduplicate per namespace we can take the highest user id if you only want to connect to one member
...
CASE 
WHEN 
m.cs_enabled_and_use = TRUE
AND
m.duo_enabled_and_use = TRUE
THEN TRUE ELSE FALSE END
AS used_both,
CASE WHEN d.ultimate_parent_namespace_id IS NULL THEN 'Trial' ELSE 'Purchaser' END AS trial_v_purchasers
FROM
mapper_ai m
LEFT JOIN 
duo_purchasers d ON d.ultimate_parent_namespace_id = m.ultimate_parent_namespace_id
GROUP BY ALL
ORDER BY 

WITH internal_issues AS (
  ...
  SELECT
    ...
    IFNULL(REPLACE(REGEXP_SUBSTR(ARRAY_TO_STRING(internal_issues.labels, ','), '\\bpriority::([0-4]$)'), 'priority::', ''),'undefined')                                                                                                                    AS priority_label,
    IFNULL(REPLACE(REGEXP_SUBSTR(ARRAY_TO_STRING(internal_issues.labels, ','), '\\bseverity::([0-9]+)'), 'severity::', ''),'undefined')                                                                                                                    AS severity_label,

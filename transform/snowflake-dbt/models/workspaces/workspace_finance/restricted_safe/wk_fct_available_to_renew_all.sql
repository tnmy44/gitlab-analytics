...
--) select * from zuora_legacy_ramps
...
--Opportunity_category is manually updated, over 90% accurate
   ...
   SELECT 
    ...
    WHERE ssp_id IS NOT NULL 
      AND mart_crm_opportunity.opportunity_category LIKE '%Ramp Deal%'---3531 rows
...
--)select * from ramp_deals_ssp_id_multiyear_linkage ---4957 with SF data only, 5042 when adding Zuora Data, Only Zuora - 1324 records
...
--) select * from subscriptions_with_ssp_id
...
--Final ATR 
    ...
    user_name,
      ...
      --country 
      --opp term 
      --tricky for ramp deals chck old code --term start --term end 
...
--select fiscal_quarter_name_fy,sum(ARR) from final group by fiscal_quarter_name_fy order by fiscal_quarter_name_fy desc

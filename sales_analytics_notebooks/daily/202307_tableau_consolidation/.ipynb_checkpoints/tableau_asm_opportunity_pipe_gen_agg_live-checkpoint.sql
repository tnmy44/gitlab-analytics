...
aggregated_base AS (SELECT
                        -------
                        -------
                        -- DIMENSIONS
                        'pipe gen net arr aggregated'               AS record_type,
                    ...
                    WHERE pipeline_created_fiscal_quarter_date IS NOT NULL
...
base_key AS (SELECT DISTINCT a.owner_id,
                             a.opportunity_owner,
    ...
    SELECT
        ...
        base_key.report_date AS pipeline_created_fiscal_quarter_date,
    ...
    LEFT JOIN aggregated_base
            ON base_key.owner_id = aggregated_base.owner_id
    ...
    LEFT JOIN aggregated_base previous_quarter
            ON base_key.owner_id = previous_quarter.owner_id
    ...
    SELECT
        ...
        COALESCE(
            pipeline_created_fiscal_quarter_date <= current_fiscal_quarter_date
            AND pipeline_created_fiscal_quarter_date >= DATEADD(MONTH, -15, current_fiscal_quarter_date), FALSE
    ...
    LEFT JOIN date_details AS close_date
        ON close_date.date_actual = final.pipeline_created_fiscal_quarter_date
    WHERE (
        net_arr != 0
        OR booked_net_arr != 0

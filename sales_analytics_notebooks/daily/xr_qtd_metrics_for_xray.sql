...
aggregation AS (
    ...
    SELECT
        ...
        SUM(CASE
            WHEN
                oppty.close_fiscal_quarter_name = today.current_fiscal_quarter_name
        ...
        SUM(CASE
            WHEN
                oppty.pipeline_created_fiscal_quarter_name = today.current_fiscal_quarter_name
                AND oppty.is_eligible_created_pipeline_flag = 1
                THEN oppty.calculated_deal_count
            ELSE 0
        END) AS qtd_pipe_gen_deal_count,
        ...
        SUM(CASE
            WHEN
                oppty.close_fiscal_quarter_date = DATEADD(MONTH, 3, today.current_fiscal_quarter_date
                )
                AND oppty.is_stage_1_plus = 1
                AND oppty.is_eligible_open_pipeline_flag = 1
                THEN oppty.calculated_deal_count
        ...
        SUM(CASE
            WHEN
                oppty.close_fiscal_quarter_date = DATEADD(MONTH, 6, today.current_fiscal_quarter_date
                )
                AND oppty.is_eligible_open_pipeline_flag = 1
                AND oppty.is_stage_1_plus = 1
                THEN oppty.calculated_deal_count
        ...
        SUM(CASE
            WHEN
                oppty.close_fiscal_year = today.current_fiscal_year
                AND oppty.is_stage_1_plus = 1
                AND oppty.is_eligible_open_pipeline_flag = 1
                THEN oppty.calculated_deal_count
            ELSE 0
...
report_opportunity_metrics_qtd AS (
    ...
    SELECT
        ...
        COALESCE(agg_demo_keys.key_bu_ot, 'other')                       AS key_bu_ot,
        COALESCE(agg_demo_keys.key_bu_sqs, 'other')                      AS key_bu_sqs,
        ...
        COALESCE(agg_demo_keys.key_bu_subbu_division_asm, 'other')       AS key_bu_subbu_division_asm

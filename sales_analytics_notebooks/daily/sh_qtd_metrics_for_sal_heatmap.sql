WITH date_details AS (
  ...
  FROM prod.workspace_sales.date_details
    ...
    SELECT day_of_fiscal_quarter_normalised AS current_day_of_fiscal_quarter_normalised,
    ...
    WHERE is_active = 1
    AND (lower(title) like '%strategic account%'
...
, sfdc_opportunity_xf AS (
  ...
  SELECT opp.*,
        acc.is_key_account
  FROM prod.restricted_safe_workspace_sales.sfdc_opportunity_xf opp
  INNER JOIN prod.restricted_safe_common_mart_sales.mart_crm_account acc
      ...
      SUM(CASE
       ...
       SUM(CASE
        ...
        SUM(CASE
         ...
         SUM(CASE
            WHEN open_1plus_net_arr < 500000
...
, pipe_gen_metrics AS (
  ...
  WHERE is_eligible_created_pipeline_flag = 1
    AND pipeline_created_fiscal_quarter_date = (SELECT first_day_of_fiscal_quarter
                                                  FROM date_details
                                                  WHERE date_actual = CURRENT_DATE
                                                  GROUP BY 1)
  GROUP BY 1
...
, duplicated_key_fields AS (
 ...
 FROM unique_key_fields key_fields
...
SELECT
   ...
   FROM base_fields
   INNER JOIN dim_users user
      ON user.user_id = base_fields.owner_id
   LEFT JOIN current_quarter opp
    ON base_fields.owner_id = opp.owner_id
   LEFT JOIN pipe_gen_metrics pipeline_created

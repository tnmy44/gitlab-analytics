...
def refactor_tickets_read_gcp():
    ...
    for blob in BUCKET.list_blobs(
        prefix="meltano/tap_zendesk__sensitive/tickets/"
    ):
...
def refactor_tickets(df_tickets: pd.DataFrame):
    ...
    storage_client = storage.Client(credentials=scoped_credentials)
    BUCKET = storage_client.get_bucket(bucket_name)
    for blob in BUCKET.list_blobs(prefix='meltano/tap_zendesk__sensitive/ticket_fields/'):
        print(blob.name)
        if blob.name.endswith('.jsonl'):
            #open a csv file and put contents of it in dataframe
            df_ticket_fields_extracted = pd.read_json(io.BytesIO(blob.download_as_string()), lines=True)
    ...
    for ind in df_ticket_fields_extracted.index:
        ...
        if type(CUSTOM_FIELD_OPTIONS) is not float: # When the data is null CUSTOM_FIELD_OPTIONS is a float
    ...
    for ind in df_tickets.index:
        ...
        row_list=[ALLOW_ATTACHMENTS, ALLOW_CHANNELBACK, ASSIGNEE_ID, BRAND_ID, COLLABORATOR_IDS, CREATED_AT, CUSTOM_FIELDS_OUT ,DESCRIPTION, DUE_AT, EMAIL_CC_IDS, EXTERNAL_ID, FOLLOWER_IDS, FOLLOWUP_IDS, FORUM_TOPIC_ID, GENERATED_TIMESTAMP, GROUP_ID, HAS_INCIDENTS, ID, IS_PUBLIC, ORGANIZATION_ID, PRIORITY, PROBLEM_ID, RECIPIENT, REQUESTER_ID, SATISFACTION_RATING, SHARING_AGREEMENT_IDS, STATUS, SUBJECT, SUBMITTER_ID, TAGS, TICKET_FORM_ID, TYPE, UPDATED_AT, URL, VIA ]
    ...
    output_df = pd.DataFrame(output_list, columns =[ 'ALLOW_ATTACHMENTS', 'ALLOW_CHANNELBACK', 'ASSIGNEE_ID', 'BRAND_ID', 'COLLABORATOR_IDS', 'CREATED_AT', 'CUSTOM_FIELDS', 'DESCRIPTION', 'DUE_AT', 'EMAIL_CC_IDS', 'EXTERNAL_ID', 'FOLLOWER_IDS', 'FOLLOWUP_IDS', 'FORUM_TOPIC_ID', 'GENERATED_TIMESTAMP', 'GROUP_ID', 'HAS_INCIDENTS', 'ID', 'IS_PUBLIC', 'ORGANIZATION_ID', 'PRIORITY', 'PROBLEM_ID', 'RECIPIENT', 'REQUESTER_ID', 'SATISFACTION_RATING', 'SHARING_AGREEMENT_IDS', 'STATUS', 'SUBJECT', 'SUBMITTER_ID', 'TAGS', 'TICKET_FORM_ID', 'TYPE', 'UPDATED_AT', 'URL', 'VIA'])

            ...
            if 'field_name' in key:
                if key['field_name'] in ('sla_policy','priority','is_public'):
                    ...
                    if key['id'] is None:
                        ...
                        id=key['id']
                    row_list=[author_id,created_at,field_name,type,value,id,ticket_id,via]
                    output_list.append(row_list)

    # add output_list to output_df
    output_df = pd.DataFrame(output_list, columns =['author_id','created_at','field_name','type','value','id','ticket_id','via'])
